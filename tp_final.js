...
function setup() {
  ...
  for (let i=0; i<23; i++) {
    ...
    texto [11]='McClean recomienda apagar la habitación e increpa a George por haber dejado que la casa criara a su familia, ahora sus hijos no lo respetan y usan la habitacion de manera hostil. ¿Que debería de hacer George?';
...
function draw() {
  ...
  if (click == 0) { // Botones de inicio y créditos
    fill(0, 0, 0, 90);
    rect(posXBotonInicial, posYBotonInicial, tamBoton * 3, tamBoton, 10);
    rect(posXBoton2Creditos, posYBoton2Creditos, tamBoton * 3, tamBoton, 10);
  ...
  if (click == 3) { // Botones de decisión
    fill(0, 0, 0, 120);
  ...
  if (click == 11) {
    fill(0, 0, 0, 120);
    rect(posXBoton - 50, posYBoton + 20, tamBoton * 4, tamBoton, 10);
    rect(posXBoton2 - 50, posYBoton2 + 20, tamBoton * 4, tamBoton, 10);
    fill (255);
    textSize (19);
    textAlign (CENTER, CENTER);
    text ("Escuhar a McClean", posXBoton + tamBoton, posYBoton + tamBoton / 2 + 20);
    text ("Ignorar a McClean", posXBoton2 + tamBoton, posYBoton2 + tamBoton / 2 + 20);
  }
    if (click == 14) {
    fill(0, 0, 0, 120);
    rect(posXBoton - 50, posYBoton, tamBoton * 4, tamBoton, 10);
    ...
    textSize (19);
    textAlign (CENTER, CENTER);
    text ("Escuhar a Lydia", posXBoton + (tamBoton), posYBoton + (tamBoton / 2));
    text ("Ignorar a Lydia", posXBoton2 + (tamBoton), posYBoton2 + (tamBoton / 2));
  }
}
function mousePressed () {
  if (click == 0) {
    // BOTONES DE MENÚ INICIAL
    if ( mouseX > posXBotonInicial &&
  ...
  } else if (decisiones[click]) {
    // Boton 1
    if (mouseX > posXBoton - 50 &&
      mouseX < posXBoton - 50 + tamBoton * 4 &&
      mouseY > posYBoton &&
      mouseY < posYBoton + tamBoton)
    {
      click = decisiones[click][0];
    }
    // Botón 2
    if (mouseX > posXBoton2 - 50 &&
      mouseX < posXBoton2 - 50 + tamBoton * 4 &&
      mouseY > posYBoton2 &&
      mouseY < posYBoton2 + tamBoton)
    {
      click = decisiones[click][1];
    }
  } else {
    if (click < imagenes.length - 1) click++;
    else click = 0; // reinicia al final
  }
}
